...
sudo kubeadm init --control-plane-endpoint=ub01.k8s.local | tee kubeadm-output.txt
awk '/Then you can join any number of worker nodes by running the following on each as root:/,EOF' kubeadm-output.txt > k8sjoin.txt
